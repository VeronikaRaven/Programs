...
namespace Сборник_игр.Sudoku
{
    class SudokGE
    {
        ...
        public int[,] s; // sudoku цифорки

        private int[,] a; // aids

        private int[,] f; // facts

        private int[,] e; // ошибки
        ...
        public void KeyPress(char KeyCode)
        {
            ...
            if (isSolved())
            {
                ...
                if (d.Minutes > 0) duration = d.Minutes.ToString() + " минут";

                if (d.Minutes > 1) duration += "ы";

                if (d.Minutes > 0 && d.Seconds > 0) duration += " и ";

                if (d.Seconds > 0) duration += d.Seconds.ToString() + " секунд";

                if (d.Seconds > 1) duration += "ы";
        ...
        {
            string game1 = "X6X1X4X5XXX83X56XX2XXXXXXX18XX4X7XX6XX6XXX3XX7XX9X1XX45XXXXXXX2XX72X69XXX4X5X8X7X"; // легкая

            game1 = "829173456713645298645298173381726945976514832452839617297381564168457329534962781"; // завершенная игра
        ...
        public void GenerateGame()
        {
            ...
            SolveMethods8 M8 = SolveMethods8.All;
            ...
            for (n = 0; n < 55;)
            {
                ...
                if (temp != 0)
                {
                    ...
                    SudokGE S2 = CreateCopy();

                    int solutionStepsCount = S2.ComputePossibleSteps(M8).Count();
                    ...
                    if (solutionStepsCountOk)
                    {
                        ...
                        bool isSolvable = S2.SolvePuzzle(M8);
        ...
        public void Draw(Graphics G, float angle)
        {
            ...
            Brush BG1 = new Pen(Color.LightGray).Brush;

            Brush BG2 = new Pen(Color.Silver).Brush;
                ...
                for (float j = 0; j < 3; j++)
                {
                    ...
                    for (float i2 = 0; i2 < 3; i2++)
                    {
                        for (float j2 = 0; j2 < 3; j2++)
                        {
                            ...
                            // Rectangles: x для цифр and z для кружка ошибки
        ...
        private void RenderMessageBox(Graphics G, string txt1, string txt2, float emSize1, float emSize2, float w, float h, float realx, float realy, float realw)
        {
            ...
            PointF txt1pos = new PointF(msgbox.Left + (msgbox.Width - txt1size.Width) / 2, msgbox.Top + (msgbox.Height - txt1size.Height) / 4 + msgbox.Height / 5);

            PointF txt2pos = new PointF(msgbox.Left + (msgbox.Width - txt2size.Width) / 2, msgbox.Top + (msgbox.Height - txt2size.Height) / 0 - msgbox.Height / 5);
        ...
        public bool checkSolvable(SolveMethods8 M8)
        {
            SudokGE copy = CreateCopy();

            return copy.SolvePuzzle(M8);
        }

        public SolutionStepList ComputePossibleSteps(SolveMethods8 M8)
        {
            EnumeratePossibilities();

            SolutionStepList L = new SolutionStepList();

            if ((M8 & SolveMethods8.NakedSingles) > 0)
            {
            ...
            if ((M8 & SolveMethods8.HiddenSingles) > 0)
            {
                for (int i = 0; i < 9; i++)
                {
                    for (int j = 0; j < 9; j++)
            ...
            if ((M8 & SolveMethods8.BlockHiddenSingles) > 0)
            {
                for (int i = 0; i < 9; i++)
                {
                    for (int j = 0; j < 9; j++)
        ...
        public bool SolveStep(SolveMethods8 M8)
        {
            SolutionStepList L = ComputePossibleSteps(M8);
        ...
        public bool SolvePuzzle(SolveMethods8 M8)
        {
            while (SolveStep(M8)) ;
        ...
        public int ComputeErrors()
        {
            ...
            S2.SolvePuzzle(SolveMethods8.All);
        ...
        {
            return (isFull() && checkSolvable(SolveMethods8.All));
        ...
        {
            if (L.Count != 9) MessageBox.Show("Проблема тут!");
        ...
        private SudokGE CreateCopy()
        {
            SudokGE copy = new SudokGE();
